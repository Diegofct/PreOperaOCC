...
- **La asignación de vehículos la decide la administración desde la web, y el móvil no tiene respaldo.** Un operador sin ninguna asignación vigente ve a quién pedírsela y no puede levantar ningún preoperacional. Hasta la spec 012 podía escoger él mismo y la fila quedaba `autoasignada` esperando confirmación; la regla de entonces decía «no bloquees al operador», porque un operador bloqueado arranca la máquina sin preoperacional. **Ese riesgo sigue siendo real y se asumió a conciencia**: una máquina operada por alguien a quien nadie se la asignó es un acta que no se sostiene, y OCC prefiere una llamada al residente a un preoperacional sin respaldo. La salida cuando alguien queda sin máquina es el panel, no el celular.
...
- **Un parte cerrado no se edita: se anula y se abre otro**, con motivo escrito. Igual que un preoperacional firmado. Por eso el índice único `(obra_id, fecha)` de `partes_de_obra` es **parcial** sobre `anulado_en is null` — sin ese predicado, anular dejaría ese día bloqueado para siempre. La tabla `bitacoras`, la del formato por máquina, se conserva con su propio índice `(vehiculo_id, fecha)`: ya no se escribe, pero el panel muestra sus filas de solo lectura al pie del parte del día.
...
- **No edites a mano** `src/features/checklists/plantillas/*.json`, `src/shared/catalogos/presupuesto.json`, `src/shared/catalogos/materiales.json` ni `drizzle/`: se generan (`npm run formatos`, `npm run presupuesto`, `npm run materiales`, `npm run db:generate`).
